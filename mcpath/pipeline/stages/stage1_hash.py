...
Specification:
- On initial trusted discovery, canonicalize definition (recursively sorted dictionary keys) and compute SHA-256 hash.
- On every later call:
    1. Identify server + tool
    2. Retrieve current tool definition from cache
    3. Canonicalize + SHA-256
    4. Retrieve approved hash from PostgreSQL
    5. Compare:
        - Mismatch -> BLOCK immediately, no downstream call, persist security event
        - Match -> PASS, persist result, continue pipeline
        - Unapproved / DB Failure -> Fail-closed BLOCK (never execute blindly)
"""
...
def compute_tool_hash(definition: Dict[str, Any]) -> str:
    """Compute SHA-256 hex digest of canonicalized tool definition."""
...
def canonicalize_and_hash(definition: Dict[str, Any]) -> Tuple[str, str]:
    """Return both canonical JSON string and SHA-256 hash."""
    canonical_json = canonicalize_tool_definition(definition)
...
class Stage1HashCheck(BasePipelineStage):
    """Stage 1: Runtime Tool Integrity Hash Check with PostgreSQL verification."""
    ...
    async def process_request(self, context: PipelineContext) -> StageResult:
        """Verify live tool definition against PostgreSQL approved hash."""
        ...
        if not tool_def:
            logger.error("Stage 1 Fail-Closed: Missing tool definition for '%s' on server '%s'", tool_name, server_name)
            context.event_record.hash_matched = False
            return StageResult(
                stage_name=self.name,
                passed=False,
                hard_block=True,
                explanation=f"Stage 1 Fail-Closed: Tool definition not found for '{tool_name}' (unapproved tool)",
                metadata={"reason": "missing_tool_definition"}
            )
        ...
        # 4. Fail-closed if no approved hash is recorded in DB
        if not expected_hash:
            logger.warning("Stage 1 Fail-Closed: No approved hash registered for '%s:%s'", server_name, tool_name)
            context.event_record.hash_matched = False
            return StageResult(
                stage_name=self.name,
                passed=False,
                hard_block=True,
                explanation=f"Stage 1 Fail-Closed: Tool '{tool_name}' has no active approved hash record in database",
                metadata={"observed_hash": observed_hash, "expected_hash": None}
            )
        ...
        return StageResult(
            ...
            passed=True,
            hard_block=False,
            explanation=f"Stage 1 Hash Check Passed: SHA-256 integrity hash matched approved definition ({observed_hash[:12]}...)",
            metadata={"expected_hash": expected_hash, "observed_hash": observed_hash}
        )
